...
const {app, server} = require('../index'); // harus export 'app' dari app.js (bukan .listen langsung!)
...
describe('Integration Test - CartService <-> ProductService', () => {
  ...
  afterAll(async () => {
    server.close(); // penting, agar test exit normal
  });


  const userId = 1; // userId test (pastikan tidak bentrok data user lain)
  const email = 'dedihandoko7@gmail.com';
